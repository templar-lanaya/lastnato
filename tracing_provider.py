...
2. `OTelCallbackHandler` -- a LangChain callback handler, import-guarded
   so this module doesn't hard-require langchain_core. Relevant for
   investor_tools_v2.py's LangChain @tool objects; NOT relevant to the
   Foundry Persistent Agent path, which doesn't run through LangChain
   callbacks at all.
"""
...
# --------------------------------------------------------------------------
# LangChain callback handler -- import-guarded. Only relevant to
# investor_tools_v2.py's LangChain @tool path, NOT the Foundry Persistent
# Agent path (which doesn't route through LangChain callbacks at all).
# Everything above this line works with zero LangChain dependency.
# --------------------------------------------------------------------------
try:
    ...
    BaseCallbackHandler = object  # so the class body below still parses


if _LANGCHAIN_AVAILABLE:

    class OTelCallbackHandler(BaseCallbackHandler):
        """Turns LangChain's run tree into OpenTelemetry spans. LangChain
        hands every callback a run_id and a parent_run_id -- that pair IS
        a tree; this mirrors it into OTel spans.
        """
        ...
        def _start(self, run_id: UUID, parent_run_id: UUID | None, name: str, kind: SpanKind):
            ...
            span = self._tracer.start_span(name, context=ctx, kind=kind)
            span.set_attribute("interaction_id", current_interaction())
            if current_agent():
                span.set_attribute("gen_ai.agent.name", current_agent())
        ...
        def on_chain_start(self, serialized, inputs, *, run_id, parent_run_id=None, **kwargs):
            ...
            self._start(run_id, parent_run_id, f"chain {name}", SpanKind.INTERNAL)
        ...
        def on_chat_model_start(self, serialized, messages, *, run_id, parent_run_id=None, **kwargs):
            ...
            model = params.get("azure_deployment") or params.get("model_name") or "unknown"
            span = self._start(run_id, parent_run_id, f"chat {model}", SpanKind.CLIENT)
            span.set_attribute("gen_ai.operation.name", "chat")
            span.set_attribute("gen_ai.provider.name", "azure.ai.openai")
            span.set_attribute("gen_ai.request.model", model)
            if params.get("temperature") is not None:
                span.set_attribute("gen_ai.request.temperature", params["temperature"])
        ...
        def on_llm_end(self, response: "LLMResult", *, run_id, **kwargs):
            ...
            if span is not None:
                ...
                if tool_calls:
                    span.set_attribute(
                        "gen_ai.response.tool_calls",
                        json.dumps([{"name": tc["name"], "arguments": tc["args"]} for tc in tool_calls]),
                    )
            self._end(run_id)
        ...
        def on_tool_end(self, output, *, run_id, **kwargs):
            span = self._spans.get(run_id)
            if span is not None:
                span.set_attribute("gen_ai.tool.result.size", len(str(output)))
            self._end(run_id)
