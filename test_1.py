...
# FIX (reverted the agent_framework-native tool from an earlier pass): evaluate_policy_rule is
# meant to be shared over MCP by every agent that needs it, not duplicated as a native tool per
# consumer -- so policy evaluation is reached the same way account_agent.py already reaches it:
# through the MCPTool below, talking to the shared "shared-tools" MCP server (tools_1.py --http).
from tools_1 import get_account_summary, get_positions, get_recent_transactions, get_account_registrations
...
# FIX: this MCPTool was previously built but never added to the agent's tools list, so the
# only way to reach policy rules was a bare, undocumented `evaluate_rule` HTTP-hop tool with no
# name/description/Annotated hints -- the model had no way to know which rule_id strings were
# valid. evaluate_policy_rule is exposed over this same MCP server tools_1.py already runs
# (`python tools_1.py --http`), and its description now spells out every valid rule_id.
tool_1 = MCPTool(
...
accountAgent= Agent(
            ...
            tools=[tool_1, get_account_summary, get_positions, get_recent_transactions, get_account_registrations ],
)
