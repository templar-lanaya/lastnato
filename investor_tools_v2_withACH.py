...
def evaluate_policy_rule(
    rule_name: Annotated[str, "One of: fee_waiver, wire_callback_required, ach_outgoing_limit, "
    ...
    if rule_name == "ach_outgoing_limit":
        ...
        # Single-amount check only -- NOT an aggregate of today's ACH
        # activity, unlike mobile_check_deposit_limit. Our Transaction
        # schema has no incoming/outgoing direction field, so summing
        # stored "ach" transactions would risk mixing both directions
        # together. Add a direction field to Transaction (and seed.py)
        # before trying to aggregate this one correctly.
    ...
    if rule_name == "ach_incoming_limit":
        ...
        # Same limitation as ach_outgoing_limit above -- single amount only.
