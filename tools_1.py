...
# FIX (test_1.py rule recognition, from an earlier pass): the MCP-decorated evaluate_policy_rule
# above is not an agent_framework tool, so it can't be passed into Agent(tools=[...]) directly.
# This is the same thin wrapper, built with agent_framework's @tool decorator instead, with a
# description that spells out every valid rule_id for the model.
@tool(
    name="evaluate_policy_rule",
    description=(
